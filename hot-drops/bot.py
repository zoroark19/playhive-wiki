...
ARTIST_CANVAS_AUTHORIZED_USER_IDS = {1120916116328435812, 690253596964815015}
...
TROPICAL_TIDE_CAPE_AUTHORIZED_USER_IDS = {1120916116328435812, 690253596964815015}
TROPICAL_TIDE_CAPE_TRIGGER_PREFIX = "%"
...
OTHER_TRACKED_ITEMS = PLUSHIES + COSTUMES + \
    [ARTIST_CANVAS_ITEM, TROPICAL_TIDE_CAPE_ITEM]
...
async def handle_tropical_tide_cape_submission(client, message, raw_content):
    gamertag = raw_content[len(TROPICAL_TIDE_CAPE_TRIGGER_PREFIX):].strip()

    if not gamertag or len(gamertag) > MAX_GAMERTAG_LENGTH:
        await message.add_reaction(EMOJI_NOT_FOUND)
        return

    owners = load_item_owners(TROPICAL_TIDE_CAPE_ITEM)
    _, existing = find_username_match(owners, gamertag)
    if existing is not None:
        await message.add_reaction(EMOJI_ALREADY_OWNED)
        await message.add_reaction(item_emoji(TROPICAL_TIDE_CAPE_ITEM))
        return
    ...
    save_item_owners(TROPICAL_TIDE_CAPE_ITEM, owners)

    print(f"[owners] manually added {api_username} to "
          f"{item_source_label(TROPICAL_TIDE_CAPE_ITEM)} via {message.author} "
          f"(id={message.author.id}), uuid={api_uuid}")
    await message.add_reaction(item_emoji(TROPICAL_TIDE_CAPE_ITEM))

    if is_new:
        total = record_submission_credit(
            message.author.id, str(message.author), TROPICAL_TIDE_CAPE_ITEM["name"])
        print(f"[submissions] +1 credit ({TROPICAL_TIDE_CAPE_ITEM['name']}) to "
              f"{message.author} (id={message.author.id}), new total={total}")
        await update_stats_message(client)
...
async def handle_message(client, message):
    ...
    if (message.author.id in ARTIST_CANVAS_AUTHORIZED_USER_IDS
            and stripped.startswith(ARTIST_CANVAS_TRIGGER_PREFIX)):
        await handle_artist_canvas_submission(client, message, stripped)
        return

    if (message.author.id in TROPICAL_TIDE_CAPE_AUTHORIZED_USER_IDS
            and stripped.startswith(TROPICAL_TIDE_CAPE_TRIGGER_PREFIX)):
        await handle_tropical_tide_cape_submission(client, message, stripped)
        return
...
def main():
    ...
    ensure_owners_file_exists(TROPICAL_TIDE_CAPE_ITEM)
